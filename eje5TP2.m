...
digits(5)%necesario
syms X2%necesario
funcion2= 1/(((X2)));%la funcion a integrar0 REVISAR
limiteinf= (1-x);%limite inferior
limitesup= 1;%limite superior/0.9
f2=int(funcion2, X2, limiteinf, limitesup);%(funcion , variable, limite inferior, limite superior)
integral1 =vpa(f2,5)%
Vrfp2=(v/k)*integral1%dm3
...
subplot(1,1,1)%N filas, Ncolumna , subindice del grafico
plot(V, CaCa0,'-r')% 'ko', 'markersize', 4);
...
k=(7.8*(10^9))*exp(-19220/T1);%1/s
FA0=q/PMA;%lbmol/hr
FA01=FA0*453.593*(1/3600)%mol/s
...
digits(5)%necesario
syms X1%necesario
funcion1= (1)/((1-X1));%la funcion a integrar0 REVISAR
limiteinf= 0;%limite inferior
limitesup= x;%limite superior/
f1=int(funcion1, X1, limiteinf, limitesup);%(funcion , variable, limite inferior, limite superior)
integral0 =vpa(f1,5)%
Vrfp1=(FA01/(k*CA0))*integral0%lt  Q/K pero no me gusta las unidades inglesas
Vrfp11=Vrfp1/1000;%m3

L1=(4*Vrfp11)/(pi*(d1^2))%m
...
P0=1%atm
q0=FA01/CA0
%Q2=q0*(P0/P)*(T1/T0)*(1+landa*x2)

titaj=1;
%titai=foi/fa0
vj=-1;
%Donde ?j representa el cociente de coeficientes estequiométricos en función del reactivo limitante 
%(negativo para reactivos y positivo para productos): 
ya0=1;

%resuelve integrales
digits(5)%necesario
syms X2%necesario
funcion2= ((1+(ya0*landa*X2)))/(titaj+(vj*X2));%la funcion a integrar0 REVISAR
limiteinf= 0;%limite inferior
limitesup= 0.9;%limite superior/
f2=int(funcion2, X2, limiteinf, limitesup);%(funcion , variable, limite inferior, limite superior)
integral2 =vpa(f2,5)%
Vrfp2=(q0/(k))*integral2%lt  Q/K pero no me gusta las unidades inglesas
Vrfp21=Vrfp2/1000%m3
L1=(4*Vrfp21)/(pi*(d1^2))%m
